...
* At the beginning of each turn, seeker can observe a number of squares. 
  ...
  * Otherwise, hider is not in the square, and all observed squares' probabilities are collapsed to 0%. The previous probability of observed square is diffused to all non-observed squares.
* At the end of each turn, hider's probability diffuses:
  * If a square is adjacent to exit squares, probability of that square is evenly spread to all exit squares.
  * Otherwise, in each square is diffused to adjacent non-observed squares.

Diffusion is calculated by taking gradient of adjacent non-observed squares, and updating the score of each square to the average of adjacent gradients.
